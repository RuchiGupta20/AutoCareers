  ...
  return (
    <div className="max-w-5xl mx-auto p-4 bg-gray-50 min-h-screen">
      ...
      <div className="space-y-4">
        {currentJobs.map(job => (
          <div key={job.id} className="bg-white rounded-lg shadow border border-gray-100 p-4 flex items-center justify-between">
            <div>
              <p className="font-semibold">{job.company}</p>
              <p className="text-gray-600 text-sm">{job.role}</p>
              <p className="text-gray-500 text-xs">{job.location}</p>
              <p className="text-gray-400 text-xs mt-1">
                Match Score: {Math.round(job.similarity_score * 100)}%
              </p>
            </div>
